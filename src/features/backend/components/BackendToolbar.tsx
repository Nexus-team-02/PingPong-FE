  ...
  return (
    <div className='flex items-center justify-between mb-8'>
      ...
      <div className='flex items-center gap-3'>
        ...
        <button
          onClick={onAuthorize}
          className={`cursor-pointer flex items-center gap-1 rounded-md border px-2 py-1 text-sm font-medium transition-colors ${
            token
              ? 'border-api-green bg-api-green-sub text-api-green hover:bg-api-green/10' // 토큰 있을 때 (성공 색상)
              : 'border-black text-black hover:bg-gray-100/50'
          }`}
        >
          {token ? 'Authorized' : 'Authorise'}
          <Lock className={`w-4.5 h-4.5 ${token ? 'text-api-green' : ''}`} />
        </button>
